...
2. *Can you then calculate the standard error and then a 95% confidence inteval?*

Note: Standard error (se) is $\frac{sd}{\sqrt{n}}$ and ci is $\frac{t-statistic}{se}$
...
functions. This results in more concice, readable code. When reading the convention is to pronounce %>% as 'then'.
...
```{r}
  ggplot(delayed_flights, aes(Month, delay_mean)) +
    ...
    geom_errorbar(aes(ymin = delay_mean - ci, ymax = delay_mean + ci)) 
```
